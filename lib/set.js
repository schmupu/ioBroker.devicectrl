...
class SetStatesAsync {
  ...
  async setMemValueByRulename(rulename) {
    try {
  ...
  setMemValue(id, value) {
    this.memvalue[id] = value;
  }

  getMemValue(id) {
    return this.memvalue[id];
  }
  ...
  async addRule(rule) {
    try {
      if (rule && rule.rulename) {
        ...
        this.adapter.log.debug('Add Ids for ' + rule.rulename + ' - ' + JSON.stringify(rule._ids));
        this.ruleset[rule.rulename] = rule;
        await this.setMemValueByRulename(rule.rulename);
      ...
      (async () => {
        try {
          if (id && state) {
            ...
            let oldValue = this.getMemValue(id);
            this.setMemValue(id, value);
            if (rules && rules.length > 0 && value !== oldValue) {
              for (let i in rules) {
                let rule = rules[i];
            ...
            try {
              ...
              for (let i in ids) {
                ...
                if (state && state.val !== value) {
                  ...
                  if (first) {
                    this.adapter.log.info(`State change for '${rulename}' on '${eventname}' to '${value}'`);
                    first = false;
                  }
                  this.adapter.log.info(`${name}' will be changed from '${state.val}' to '${value}'.`);
                  if (!this.simulation) {
