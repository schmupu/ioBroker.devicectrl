 ...
 * @param {string} time in format 'hh:mm' or 'hh:mm:ss' 
 */
...
function delayCalc(rulename, rule, value) {
  if (typeof delayCalc.arrays === 'undefined') {
    delayCalc.arrays = {};
  }
  if (!delayCalc.arrays[rulename]) {
    delayCalc.arrays[rulename] = {};
  }
  if (!delayCalc.arrays[rulename][rule]) {
    delayCalc.arrays[rulename] = {};
    if (value === undefined || value === null || rule == undefined) {
      ...
      now = formatTimeStr(now);
      if(rgtmp.length === 2) {
        value = timePlusTime(rgtmp[1], '-' + rgtmp[0]);
        value = timePlusRandomTime('delay_' + rulename + '_' + rule, now, value);
        value = timePlusTime(rgtmp[0], value);
        delayCalc.arrays[rulename][rule] = value;
      } else {
        delayCalc.arrays[rulename][rule] = timePlusTime(now, value);
      }
    }
  }
  let now = dateToTimeStr();
  now = formatTimeStr(now);
  return delayCalc.arrays[rulename][rule] <= now;
}
  ...
  async executeRuleAsync(rule) {
    try {
      ...
      if (ruleCopyRules && ruleCopyRules.regel) {
        let d = ruleCopyRules.rule.find((x) => x.name === ruleCopyRules.regel);
        d = d ? d.delay : undefined;
        let dl = delayCalc(ruleCopyRules.rulename, ruleCopyRules.regel, d);
        if ((ruleCopyRules.oldRegel != ruleCopyRules.regel || ruleCopyRules.oldValue != ruleCopyRules.value) && dl) {
          ...
          if (!this.simulation) {
            ...
            if (ruleCopyRules.id && ruleCopyRules.value !== undefined && ruleCopyRules.value !== null) {
              ...
              // this.adapter.log.info(ruleCopyRules.rulename + ', alte Regel ' + ruleCopyRules.oldRegel + ', neue Regel ' + ruleCopyRules.regel + ', von altem Wert ' + ruleCopyRules.oldValue + ' auf neuen Wert ' + ruleCopyRules.value + '. Id Fehlt um Wert zu setzen.');
              return ruleCopyRules;
            }
          } else {
