...
// **************************************************************************
function getTimeInMaxMinRange(t, range) {
...
function getTimeFromRange(obj) {
    ...
    if (ttmp.length == 2) {
        ...
        let t2 = ttmp[1];
        t1 = checkSuncalc(latitude, longitude, t1);
        // t1 = getTimeInMaxMinRange(t1, range);
        t2 = checkSuncalc(latitude, longitude, t2);
        // t2 = getTimeInMaxMinRange(t2, range);
        ...
        // t = getTimeInMaxMinRange(t, range);
        return t;
    }
}
...
function inTime(rulename, obj, feiertage, latitude, longitude) {
    ...
    if (obj) {
        ...
        if (!obj.from && obj.to && obj.duration) {
            ...
            // obj.from = getTimeInMaxMinRange(obj.from, obj.range);
        }
        if (obj.from && !obj.to && obj.duration) {
            obj.to = timePlusTime(obj.from, obj.duration);
            // obj.to = getTimeInMaxMinRange(obj.to, obj.range);
        }
        ...
        let value = getTimeInMaxMinRange(obj.from, obj.range) && getTimeInMaxMinRange(obj.to, obj.range);
