...
function getStatesByObjectArray(adapter, objArray, callback) {
    ...
    if (!objArray) return callback && callback();
...
class RulesControler {
    ...
    evalStates(rule, callback) {
        if (!rule) return callback && callback();
        ...
        try {
            ...
            this.adapter.log.error("Error in getStatesByObjectArray / Promise");
            callback && callback();
        }
    }
    ...
    evalQuery(rule, callback) {
        ...
        for (let i in rule.rule) {
            ...
            try {
                ...
                this.adapter.log.error("Error in vm.runInNewContext");
            }
