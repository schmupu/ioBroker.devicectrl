...
class RulesControler {
    ...
    evalStates(rule, callback) {
        ...
        if (rule && rule.state) {
            for (let i in rule.state) {
                ...
                if (!obj.hasOwnProperty("name")) return callback && callback("Missing Parmater Name in one of the States in " + rule.rulename);
                if (!obj.hasOwnProperty("id")) return callback && callback("Missing Parmater id in State " + rule.rulename + " / " + obj.name);
            }
    ...
    evalRules(rule, callback) {
        if (!rule || !rule.rule) return callback && callback("Rule missing");
        ...
        for (let i in rule.rule) {
            let obj = rule.rule[i];
            if (!obj.hasOwnProperty("name")) return callback && callback("Missing Parmater Name in one of the Rules in " + rule.rulename);
            if (!obj.hasOwnProperty("query")) return callback && callback("Missing Parmater query in Rule " + rule.rulename + " / " + obj.name);
            if (!obj.hasOwnProperty("id")) return callback && callback("Missing Parmater id in Rule " + rule.rulename + " / " + obj.name);
            if (!obj.hasOwnProperty("value")) return callback && callback("Missing Parmater value in Rule " + rule.rulename + " / " + obj.name);
    ...
    executeRule(rule, callback) {
        ...
        this.evalTimeStates(ruleCopy, (err, values) => {
            if (err) {
                callback && callback(err, values);
            } else {
                ruleCopy = values;
                this.evalRules(ruleCopy, (err, val) => {
                    if (err) {
                            ...
                            if (val.oldRegel != val.regel || val.oldValue != val.value) {
                                if (!this.simulation) {
                                    ...
                                    if (val.id) {
                                        ...
                                        this.adapter.log.info(val.rulename + ", alte Regel " + val.oldRegel + ", neue Regel " + val.regel + ", von altem Wert " + val.oldValue + " auf neuen Wert " + val.value + ". Id nicht gesetzt.");
                                        callback && callback(err, val);
                                    }
                                } else {
