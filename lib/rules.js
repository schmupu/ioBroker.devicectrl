...
class RulesControler {
  ...
  addObject(rule) {
    ...
    this.adapter.setObjectNotExists(deviceid, {
      type: 'device',
      common: {
        name: 'Rules'
      }
    ...
    if (rule && rule.rulename) {
      ...
      this.adapter.setObjectNotExists(channelid, {
        type: 'channel',
        common: {
  ...
  delObjects() {
    ...
    this.adapter.getChannelsOf(deviceid, (err, channel) => {
      for (let i in channel) {
        ...
        if (found == false) {
          this.adapter.deleteChannel(deviceid, channelname);
        }
      }
    });
  }
  ...
  evalTime(rule, callback) {
    // if (!rule) return null;
    if (rule) {
      for (let i in rule.time) {
        let obj = rule.time[i];
        if (!obj.hasOwnProperty('name')) return callback && callback('Missing parmater name in section time of ' + rule.rulename);
      }
  ...
  evalStates(rule, callback) {
    ...
    if (rule) {
      for (let i in rule.state) {
        ...
        if (!obj.hasOwnProperty('name')) return callback && callback('Missing parameter name in section state in ' + rule.rulename);
        if (!obj.hasOwnProperty('id')) return callback && callback('Missing parameter id in section state of ' + rule.rulename + ' in ' + obj.name);
      }
  ...
  evalTimeStates(rule, callback) {
    // if (!rule) return callback && callback('Rule missing in evalTimeStates');
    if (rule) {
      this.adapter.log.debug('Staring evalTimeStates with rule: ' + JSON.stringify(rule));
      // rule = this.evalTime(rule);
      this.evalTime(rule, (err, rule) => {
        if (err) {
          callback && callback(err);
        } else {
          this.evalStates(rule, (err, rule) => {
            if (err) {
              callback && callback(err);
            } else {
              callback && callback(err, rule);
  ...
  evalRules(rule, callback) {
    if (rule) {
      ...
      for (let i in rule.rule) {
        let obj = rule.rule[i];
        if (!obj.hasOwnProperty('name')) return callback && callback('Missing parameter name in section rule of ' + rule.rulename);
        if (!obj.hasOwnProperty('query')) return callback && callback('Missing parameter query in section rule of  ' + rule.rulename + ' in ' + obj.name);
        if (!obj.hasOwnProperty('id')) return callback && callback('Missing parameter id in section rule of ' + rule.rulename + ' in ' + obj.name);
        if (!obj.hasOwnProperty('value')) return callback && callback('Missing parameter value in section rule of  ' + rule.rulename + ' in ' + obj.name);
        ...
        if (!found && sandbox.hasOwnProperty(obj.name) && sandbox[obj.name] == true) {
          ...
          // rule.oldRegel = rule.regel != undefined ? rule.regel : obj.name;
          rule.regel = obj.name;
          rule.oldValue = rule.value;
          // rule.oldValue = rule.value != undefined ? rule.value : obj.value;
  ...
  executeRule(rule, callback) {
    ...
    this.evalTimeStates(ruleCopy, (err, ruleCopyTimes) => {
      if (err) {
        callback && callback(err, ruleCopyTimes);
      } else {
        this.evalRules(ruleCopyTimes, (err, ruleCopyRules) => {
          if (err) {
            callback && callback(err, ruleCopyRules);
          } else {
            if (ruleCopyRules) {
              ...
              if (ruleCopyRules.oldRegel != ruleCopyRules.regel || ruleCopyRules.oldValue != ruleCopyRules.value) {
                if (!this.simulation) {
                  // Set Sate
                  if (ruleCopyRules.id) {
                    this.adapter.setForeignState(ruleCopyRules.id, ruleCopyRules.value, (err) => {
                      if (!err) {
                    ...
                    this.adapter.setForeignState(stateid, ruleCopyRules.regel);
                    stateid = channelid + '.ruleset';
                    this.adapter.setForeignState(stateid, JSON.stringify(rule));
                    stateid = channelid + '.value';
                    this.adapter.setForeignState(stateid, ruleCopyRules.value);
                    stateid = channelid + '.id';
                    this.adapter.setForeignState(stateid, ruleCopyRules.id);
                  } else {
        ...
        if (ruleCopyRules.oldRegel != ruleCopyRules.regel || ruleCopyRules.oldValue != ruleCopyRules.value) {
          if (!this.simulation) {
            ...
            if (ruleCopyRules.id) {
              ...
              let channelid = deviceid + '.' + rule.rulename.replace(/[^0-9a-zA-Z]/g, '');
              let stateid = channelid + '.rule';
              let ids = (typeof ruleCopyRules.id === 'object') ? ruleCopyRules.id : Array(ruleCopyRules.id);
              for (let id in ids) {
                await this.adapter.setForeignStateAsync(id, ruleCopyRules.value);
              }
              this.adapter.log.info(ruleCopyRules.rulename + ', alte Regel ' + ruleCopyRules.oldRegel + ', neue Regel ' + ruleCopyRules.regel + ', von altem Wert ' + ruleCopyRules.oldValue + ' auf neuen Wert ' + ruleCopyRules.value + '.');
                ...
                if (ruleCopyRules.oldRegel != ruleCopyRules.regel || ruleCopyRules.oldValue != ruleCopyRules.value) {
                    if (!this.simulation) {
                        ...
                        if (ruleCopyRules.id) {
                            ...
                            for(let id in ids) {
                                let stateId = ids[id];
                                this.adapter.log.info('Changing value for Id: ' + stateId + ' in rule ' + ruleCopyRules.rulename);
                                await this.adapter.setForeignStateAsync(stateId, ruleCopyRules.value);
                            }
