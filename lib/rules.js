...
function getStatesByObjectArray(adapter, objArray, callback) {
  ...
  getStatesByArray(adapter, arr, (error, values) => {
    for (let i = 0; i < objArray.length; i++) {
      ...
      let obj = objArray[i];
      obj.oldValue = Object.hasOwnProperty.call(obj, 'value') ? obj.oldValue = obj.value : obj.oldValue = undefined;
      obj.value = val && Object.hasOwnProperty.call(val, 'val') ? val.val : null;
      if (Object.hasOwnProperty.call(obj, 'cmpvalue')) {
...
function buildEvalObjects(objArrays) {
  ...
  for (let i in objArrays) {
    ...
    for (let j in objArray) {
      let obj = objArray[j];
      if (Object.hasOwnProperty.call(obj, 'name') && Object.hasOwnProperty.call(obj, 'value')) {
...
class RulesControlerAsync {
  ...
  init(rule) {
    if (rule) {
      ...
      if (!Object.hasOwnProperty.call(rule, 'time')) { rule.time = {}; }
      if (!Object.hasOwnProperty.call(rule, 'state')) { rule.state = {}; }
      if (!Object.hasOwnProperty.call(rule, 'rule')) { rule.rule = {}; }
      return rule;
    }
  }
  ...
  async addObjectAsync(rule) {
    if (rule && rule.rulename) {
      try {
        ...
        if (rule.rule && rule.rule[0] && Object.hasOwnProperty.call(rule.rule[0], 'value')) {
          let typerule = typeof rule.rule[0].value;
          if (typerule) {
            id = channelid + '.value';
            await this.adapter.setObjectNotExistsAsync(id, {
              type: 'state',
  ...
  inTime(rulename, obj, feiertage) {
    if (obj) {
      ...
      obj.oldValue = Object.hasOwnProperty.call(obj, 'value') ? obj.value : undefined;
      obj.value = value;
    }
    return obj;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      if (rule) {
        for (let i in rule.time) {
          let obj = rule.time[i];
          if (!Object.hasOwnProperty.call(obj, 'name')) reject('Missing parmater name in section time of ' + rule.rulename);
        }
    ...
    return new Promise((resolve, reject) => {
      if (rule) {
        for (let i in rule.state) {
          ...
          if (!Object.hasOwnProperty.call(obj, 'name')) reject('Missing parameter name in section state in ' + rule.rulename);
          if (!Object.hasOwnProperty.call(obj, 'id')) reject('Missing parameter id in section state of ' + rule.rulename + ' in ' + obj.name);
        }
    ...
    return new Promise((resolve, reject) => {
      if (rule) {
        ...
        for (let i in rule.rule) {
          let obj = rule.rule[i];
          if (!Object.hasOwnProperty.call(obj, 'name')) reject('Missing parameter name in section rule of ' + rule.rulename);
          if (!Object.hasOwnProperty.call(obj, 'query')) reject('Missing parameter query in section rule of  ' + rule.rulename + ' in ' + obj.name);
          if (!Object.hasOwnProperty.call(obj, 'id')) reject('Missing parameter id in section rule of ' + rule.rulename + ' in ' + obj.name);
          if (!Object.hasOwnProperty.call(obj, 'value')) reject('Missing parameter value in section rule of  ' + rule.rulename + ' in ' + obj.name);
          ...
          if (!found && Object.hasOwnProperty.call(sandbox, obj.name) && sandbox[obj.name] == true) {
        ...
        if ((ruleCopyRules.oldRegel != ruleCopyRules.regel || ruleCopyRules.oldValue != ruleCopyRules.value) && delay) {
          ...
          if (!this.simulation) {
            ...
            if (ruleCopyRules.id && ruleCopyRules.value !== undefined && ruleCopyRules.value !== null) {
              ...
              let valueid = typeof ruleCopyRules.id === 'string' ? ruleCopyRules.id : JSON.stringify(ruleCopyRules.id);
              await this.adapter.setForeignStateAsync(stateid, { val: valueid, ack: true });

              stateid = channelid + '.code';
              let code = typeof ruleCopyRules.code === 'string' ? ruleCopyRules.code : JSON.stringify(ruleCopyRules.code);
              await this.adapter.setForeignStateAsync(stateid, { val: code, ack: true });

              stateid = channelid + '.sandbox';
              let sandbox = typeof ruleCopyRules.sandbox === 'string' ? ruleCopyRules.sandbox : JSON.stringify(ruleCopyRules.sandbox);
              await this.adapter.setForeignStateAsync(stateid, { val: sandbox, ack: true });
