...
// **************************************************************************
function inTimeOld(rulename, obj, feiertage, sunCalcTimes) {
...
// obj = { name: "t2", from: "19:00", to: "23:00", weekday: "Fr,Sa" }
// **************************************************************************
function inTime(rulename, obj, feiertage, sunCalcTimes) {
 
    if (obj) {
        let name = rulename + '_' + obj.name;
        if (obj.from == 'sunrise') {
            obj.from = dateToTimeStr(sunCalcTimes.sunrise);
        }
        if (obj.from == 'sunset') { obj.from = dateToTimeStr(sunCalcTimes.sunset); }
        if (obj.to == 'sunrise') { obj.to = dateToTimeStr(sunCalcTimes.sunrise); }
        if (obj.to == 'sunset') { obj.to = dateToTimeStr(sunCalcTimes.sunset); }
...
class RulesControler {
    ...
    setCoordinates(latitude, longitude) {
        ...
        this.sunCalcTimes = SunCalc.getTimes(new Date(), latitude, longitude);
    }
    ...
    evalTime(rule) {
        ...
        for (let i in rule.time) {
            let obj = rule.time[i];
            obj = inTime(rule.rulename, obj, this.feiertage, this.sunCalcTimes);
            rule.time[i] = obj;
        }
        return rule;
    }
    ...
    executeRules(callback) {
        for (let i in this.ruleset) {
            let rule = copyObject(this.ruleset[i]);
            this.evalAll(rule, (err, values) => {
                if (err) {
                    callback && callback(err, values);
                } else {
                    rule = values;
                    this.evalQuery(rule, (err, val) => {
                        if (err) {
                            callback && callback(err, val);
                        } else {
                            if (val) {
                                this.ruleset[i].oldRegel = val.oldRegel;
                                this.ruleset[i].regel = val.regel;
                                this.ruleset[i].oldValue = val.oldValue;
                                this.ruleset[i].value = val.value;

                                if (val.oldRegel != val.regel || val.oldValue != val.value) {
                                    if (val.id && !this.simulation) {
                                        // Set Sate
