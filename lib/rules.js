...
function inTime(obj) {
    ...
    if (!obj.hasOwnProperty("func")) { obj.func = function () { return true; } }

    let value = compareTime(obj.from, obj.to) && isWochetag(obj.weekday) && obj.func();
    ...
    query(objArrays, callback) {
        ...
        for (let i in objArray) {
            ...
            if (found == false && sandbox.hasOwnProperty(name) && sandbox[name] == true) {
                ...
                this.adapter.log.info("Status setzen für  " + objArrays.rulename + " auf " + objArrays.regel + " mit Wert " + objArrays.value);
            }
        }
        callback && callback(objArrays);
    }
